...
from azure.ai.face.models import (
    LivenessSessionParameters,
    LivenessWithVerifySessionParameters,
    LivenessWithVerifySessionCreateContent,
)
...
if __name__ == "__main__":
    ...
    parameters = LivenessWithVerifySessionParameters(
        device_correlation_id=TEST_DEVICE_CORELATION_ID,
        liveness_operation_mode=TEST_LIVENESS_OPERATION_MODE,
    )
    with open(sample_path, "rb") as fd:
        file = fd.read()
    sessionWithVerify = LivenessWithVerifySessionCreateContent(
        parameters=parameters, verify_image=("verify_image.jpg", file, "base64")
    )
